...
class ShaderWidget(QOpenGLWidget):
    ...
    def __init__(self, parent, shader_config):
        ...
        self.timer.timeout.connect(self._safe_update)
    ...
    def _safe_update(self):
        """Safe update that checks if widget is still valid."""
...
class OverlayBackgroundShader:
    ...
    def cleanup(self):
        """Clean up resources with defensive error handling."""
        if self.widget:
            # Mark as destroyed first to stop any pending updates
            try:
                self.widget._is_destroyed = True
            except (RuntimeError, AttributeError):
                pass
            
            # Stop timer
            try:
                if hasattr(self.widget, 'timer') and self.widget.timer:
                    self.widget.timer.stop()
                    try:
                        self.widget.timer.timeout.disconnect()
