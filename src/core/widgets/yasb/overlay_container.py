...
class OverlayContainerWidget(BaseWidget):
    ...
    def __init__(
        ...
        self._child_widget = None
        self._bar_widget_ref = None  # Use weakref to prevent preventing GC
        self._target_widget_ref = None  # Use weakref for target widget
        ...
        self._initialized = False  # Track if initialization completed
        ...
        # Connect to destroyed signal for early cleanup
        try:
    ...
    def _on_destroyed(self):
        """Handle widget destruction - called before C++ object is deleted."""
        self._is_destroyed = True
        self._is_cleaning_up = True
        # Remove all event filters immediately
    ...
    def _install_event_filters(self):
        ...
        if self._bar_widget:
            try:
                ...
                # Store weak reference to track installed filters
                self._event_filter_targets.append(weakref.ref(self._bar_widget))
        ...
        if self._target == "widget" and self._target_widget:
            ...
            if target_widget:
                try:
                    ...
                    self._event_filter_targets.append(weakref.ref(target_widget))
                    logging.debug(
                ...
                try:
                    for widget in section_container.findChildren(BaseWidget):
                        try:
                            ...
                            self._event_filter_targets.append(weakref.ref(widget))
                            logging.debug(
    ...
    def eventFilter(self, obj, event):
        """Handle events for resize tracking with safety checks."""
        # CRITICAL: Early exit if widget is being destroyed or cleaned up
        ...
        if event_type in (QEvent.Type.Resize, QEvent.Type.Move, QEvent.Type.Show, QEvent.Type.Hide):
            ...
            try:
                ...
                # Reference became invalid during check
                return False
    ...
    def closeEvent(self, event):
        """Enhanced cleanup on widget close"""
        self._is_destroyed = True
        self._is_cleaning_up = True
        try:
            self.cleanup()
        except Exception as e:
            logging.debug(f"OverlayContainerWidget: Error in closeEvent cleanup: {e}")
